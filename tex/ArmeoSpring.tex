...
The average speed and tangential velocity of each test for each subject, as well as control group average, is shown in figure \ref{}. Speed and velocity highly correlated with each other, but the gap between them decreases in time (which is also shown by path ratio in \ref{}). Hence we analyze only velocity here to avoid redundancy. 
